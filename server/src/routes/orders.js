...
router.post("/", (req, res) => {
  ...
  if (!userId) {
    ...
    if (!orderItems || orderItems.length === 0) {
      return res.status(400).json({ message: "Inga orderrader skickades till databasen" });
    }

    const order = db.prepare("INSERT INTO orders (user_id, product_id, quantity) VALUES (?, ?, ?)");
